Feature: Create an individual order
  ...
  Background:
    ...
    Scenario: creation d'une order avec succes
        When a registeredUser creates an order for the restaurant Naga with the deliveryPlace created for 13h on 01-01-2025 the current date being 01-01-2025 10:00
    ...
    Scenario: creation d'un order en erreur (date de livraison non renseignée)
    ...
    Scenario: creation d'un order en erreur (date de livraison antérieure à la date courante)
      When a registeredUser creates an order for the restaurant Naga with the deliveryPlace created for 13h on 01-01-2020 the current date being 01-01-2025 10:00
      Then the registeredUser should not have any currentOrder